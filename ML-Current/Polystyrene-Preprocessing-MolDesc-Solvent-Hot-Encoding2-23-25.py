...
# %%
#original_file = r"C:\Users\micha\OneDrive\Documents\GitHub\ActiveProjectsPrivate\ActiveFiles\Excels\LCCCSeed-ML-2-2-25-RemovedCommasFromSolventsList.xlsx"
original_file = r"C:\Users\Dillo\OneDrive\Documents\GitHub\ActiveProjectsPrivate\ActiveFiles\Excels\LCCCSeed-ML-2-2-25-RemovedCommasFromSolventsList.xlsx"

original_data = pd.read_excel(original_file)

# %%
# Column statistics related to missing percentage
...
# %%
# Column statistics related to missing percentage - Same thing but this one tracks columns to be dropped due to more than 50% missing
dropped_columns = []
column_notmissing = []
column_missing = []
total_rows = len(original_data)
...
# %%
# Prints the columns that were dropped due to missing percentage
print(dropped_columns)

# %%
# Removes columns that have more than 50% missing from original data frame
...
# %%
# These are the columns left after dropping >50% missing columns
data.columns

# %%
# These are the columns related to the website / paper draft / or reliability scoring. The Quality score itself is kept though.
...
# Drops the columns related to website / draft
...
# %%
# After Filter rows based on column: 'Polymer'
data_clean

# %%
# Path to solvent Molecular Descriptors and Morgan Finger prints in same excel file.
#solvent_path = r"C:\Users\micha\OneDrive\Documents\GitHub\ActiveProjectsPrivate\ActiveFiles\Rdkit-Descriptors\Descriptors-Excel\SolventDescriptors-2-2-25.xlsx"
solvent_path = r"C:\Users\Dillo\OneDrive\Documents\GitHub\LCCC-ML\SolventDescriptors-UPDATED-2-23-25.xlsx"
...
# %%
# Drops all the Morgan Fingerprint columns - this version only uses molecular descriptors.
"""
Cell generated by Data Wrangler.
"""
...
# Cleaned of Morgan Fingerprints
solvent_desc_clean = clean_data(solvent_desc.copy())


# %%
# Pulls the solvents column from our Seed Data containing only polystyrene entries
...
# %%
# This still has binary pairs and the outlier 100% one solvent case
solvents_clean

# %%
# Now contains only binary solvent pairs - each solvent with their own column
"""
Cell generated by Data Wrangler.
"""
...
# %%
# Creating two new dataframes to store the binary solvent pairs' solvents individually
...
# %%
# Solvent 1 is Solvents_0
combined_descriptors_1

# %%
# Solvent 2 is Solvents_1
combined_descriptors_2

# %%
# Near Crit is removed from solvents names and not tracked further.
...
# %%
# Solvent 1 is merged with the data frame containing the solvent descriptors based on names
combined_descriptors1 = combined_descriptors_1.merge(solvent_desc_clean, left_on = 'Solvents_0', right_on = 'Solvents', how='left') 


# %%
# Solvent 1 and its descriptors - also contains the matching name and smiles from solvent descriptor excel sheet
combined_descriptors1

# %%
# Solvent 2 had some issue with leading/trailing white spaces that needed to be removed.
# This or the stripping of combined_descriptors_2 may be repetitive. 
"""
Cell generated by Data Wrangler.
"""
...
# %%
# Solvent 2 is merged with the data frame containing the solvent descriptors based on names
combined_descriptors2 = combined_descriptors2_clean.merge(solvent_desc_clean, left_on = 'Solvents_1', right_on = 'Solvents', how='left') 

# %%
# Solvent 2 and its descriptors - also contains the matching name and smiles from solvent descriptor excel sheet
combined_descriptors2

# %%
# Dropping all the string columns from solvent 1 and solvent 2 dataframes that were merged with the descriptors excel.
...
# %%
# Only Numeric descriptors for solvent 1
descriptors1

# %%
# Only Numeric descriptors for solvent 2
descriptors2

# %%
# Combining of numeric descriptors of solvent1 and solvent2
combined_descriptors = descriptors1 + descriptors2

# %%
# Contains all the molecular descriptors of solvents 1 and solvent 2 combined based on row and column. 
combined_descriptors

# %%
# All these head() examples are used to check the combining was done successfully
descriptors1.head()
...
# %%
# Solvents_clean_1_reset contains only binary solvent pairs - each solvent with their own column - just has it's index reset for concatenation success
solvents_clean_1_reset = solvents_clean_1.reset_index(drop=True)

# combined_descriptors_reset Contains all the molecular descriptors of solvents 1 and solvent 2 combined based on row and column- just has it's index reset for concatenation success
combined_descriptors_reset = combined_descriptors.reset_index(drop=True)

# Now processed_data stores the binary solvent parsed column pairs with the combined_descriptors_reset which contains the combined numeric molecular descriptors of solvent 1 and solvent 2.
processed_data = pd.concat([solvents_clean_1_reset, combined_descriptors_reset], axis=1)

# %%
# Used for concatenation accuracy checking
# combined_descriptors_reset Contains all the molecular descriptors of solvents 1 and solvent 2 combined based on row and column- just has it's index reset for concatenation success
combined_descriptors_reset

# %%
# Used for concatenation accuracy checking
# processed_data stores the binary solvent parsed column pairs with the combined_descriptors_reset which contains the combined numeric molecular descriptors of solvent 1 and solvent 2.
processed_data

# %%

# Data_clean has it's index reset here, but note that it needs to undergo the same preprocessing to limit it to binary pairs of solvents.
...
#============================================================================
# Detector handling - 
# I want to retain as much experimental data as possible so Detector column was converted from strings to a count of the detectors.
# Currently used outright in model - but thought is maybe I can incorporate it into quality score or the statistical side of the ML
#============================================================================
...
# %%
#============================================================================
# Parse the 'Solvents' column into 'Solvents_1', 'Solvents_2', 'Solvents_3', 'Solvents_4'

# Note that this starts at 'Solvents_1' unlike my other solvent parsing that started at 'Solvent_0'
# Here 'Solvents_1' is Solvent 1
#============================================================================
if "Solvents" in data_clean.columns:
    max_solvents = 4  # max solvents
...
# Used with columns containing multiple numeric entries
# eg. Pore size / particle diameter / ratios
#============================================================================
...
# Removes outlier cases of pairs of 3 and 4 solvents with data_clean['Solvents_3'].isna()
# Removes outlier cases of pairs of 100% one solvent with data_clean['Solvents_2'].notna()
"""
Cell generated by Data Wrangler.
"""
...
# %%
# Contains only binary solvent pairs - but the Solvents_3 and Solvents_4 columns still exist
data_clean_1

# %%
# Removes all the empty columns related to Solvents_3 and Solvents_4
"""
Cell generated by Data Wrangler.
"""
def clean_data(data_clean_1):
    ...
    data_clean_1 = data_clean_1.drop(columns=['Solvents_1'])
    # Drop column: 'Solvents_4'
    data_clean_1 = data_clean_1.drop(columns=['Solvents_2'])
...
# This data frame is the Seed Data containing preprocessing down to binary solvent pairs - and the other preprocessing changes to Detector/ parsing of numeric columns - pore size / particle diameter etc
data_clean_2
...
# %%
# Reset index version of dataframe of the Seed Data containing preprocessing down to binary solvent pairs - and the other preprocessing changes to Detector/ parsing of numeric columns - pore size / particle diameter etc
data_clean_2_reset = data_clean_2.reset_index(drop=True)

# reset version of processed_data stores the binary solvent parsed column pairs with the combined_descriptors_reset which contains the combined numeric molecular descriptors of solvent 1 and solvent 2.
processed_data_reset = processed_data.reset_index(drop=True)

# final contains the combined molecular descriptors and our preprocessed seed data concatenated 
...
# %%
# Missing Temperature values were filled with 25
# Missing Pore size (°A)_2 and Particle diameter (μm)_2 were filled with 0 as they are non existent.
# ^ Debating 0 or a transient number -- Thoughts?
"""
Cell generated by Data Wrangler.
"""
def clean_data(final):
    # Replace missing values with 25 in column: 'Temperature (Celsius)'
    final = final.fillna({'Temperature (Celsius)': 25})
    final = final.fillna({'Pore size (°A)_2': 0})
    final = final.fillna({'Particle diameter (μm)_2': 0})
    return final

imputated = clean_data(final.copy())
...
# %%
# Now hot encoding and imputation before ML
...
# %%
# These are to be used to find only the unique solvents in our polystyrene subset
# Ensure df1 and df2 are defined correctly
df1 = pd.DataFrame({'Solvents': imputated['Solvents_0']})
df2 = pd.DataFrame({'Solvents': imputated['Solvents_1']})

# Concatenating them into a single DataFrame with 344 rows
df_combined = pd.concat([df1, df2], axis=0, ignore_index=True)

# %%
# This was needed or some solvents popped up twice during solvent hot encoding
imputated['Solvents_0'] = imputated['Solvents_0'].str.strip()
imputated['Solvents_1'] = imputated['Solvents_1'].str.strip()

# %%
# This was needed or some solvents popped up twice during solvent hot encoding
df_combined['Solvents'] = df_combined['Solvents'].str.strip()

# %%
# Data frame containing only the unique solvents in the polystyrene subset
UniqueSolvents = df_combined.drop_duplicates(ignore_index=True).reset_index(drop=True)

# Needed conversion to list for one hot encoder to use as feature
UniqueSolvents_list = UniqueSolvents['Solvents'].tolist()
...
# %%
# Stack them into ONE Series (ignore_index=True to re-label row index)
X_combined_series = pd.concat(
    [imputated['Solvents_0'], imputated['Solvents_1']],
    axis=0,
    ignore_index=True
)

# Convert that Series into a single-column DataFrame
X_combined = X_combined_series.to_frame(name='Solvents')

# Fit the OneHotEncoder on that one-column DataFrame
encoder = OneHotEncoder(
    categories=[UniqueSolvents_list], 
    handle_unknown='ignore',
    sparse_output=True
)
encoder.fit(X_combined)
X1_df = imputated[['Solvents_0']].rename(columns={'Solvents_0': 'Solvents'})
X2_df = imputated[['Solvents_1']].rename(columns={'Solvents_1': 'Solvents'})

#  Now transform each column separately (which is 1 feature)
# This is the hot encoding of the first solvent in the binary pair
X1 = encoder.transform(X1_df)

# This is the hot encoding of the second solvent in the binary pair
X2 = encoder.transform(X2_df)

# %%
# This is the hot encoding of the first solvent in the binary pair
# 172 rows with 20 columns - only 20 unique solvents in polystyrene subset
X1

# %%
# This is pulling the solvent 1 ratio and making it 1 dimensional.
ratio_1 = imputated['Solvent Ratio_1'].values.reshape(-1, 1)

# This is pulling the solvent 2 ratio and making it 1 dimensional.
ratio_2 = imputated['Solvent Ratio_2'].values.reshape(-1, 1)

# %%
# This is taking the 1 / 0 hot encoding and turning it to the solvent hot encoding value being the ratio of that solvent.
# The solvent 1 hot encoded 1 dimensional array is multiplied by the solvent 1 ratio 1 dimensional array
X1_ratio = X1.multiply(ratio_1)

# The solvent 2 hot encoded 1 dimensional array is multiplied by the solvent 2 ratio 1 dimensional array
X2_ratio = X2.multiply(ratio_2)

# %%
# This addition turns the two individual solvent hot encoding of their ratios into one array containing the hot encoding of the binary solvent pairs by their respective ratios.
X = X1_ratio + X2_ratio
X

# %%
# This converts the strange compressed matrix of dtype into an array that pandas can use.
X_dense = X.toarray()

# Encoded df merges the array of hot encoded solvents by their ratios with the encoder feature names - not lovely with the Solvents_SolventName styling
encoded_df = pd.DataFrame(X_dense, columns=encoder.get_feature_names_out())
encoded_df

# %%
# Replaces the encoder feature names headers with Unique Solvents list created earlier - allowing names to be cleaner.
encoded_df.columns = UniqueSolvents_list
encoded_df

# %%
# Index reset in preparation for concatenation with final containing the reset of the seed data and descriptors
encoded_df = encoded_df.reset_index(drop=True)
imputated = imputated.reset_index(drop=True)

# final_encoded contains hot encoded solvents by their ratios with the rest of the preprocessed seed data containing descriptors
final_encoded = pd.concat([encoded_df, imputated], axis=1)

# %%
# final_encoded contains hot encoded solvents by their ratios with the rest of the preprocessed seed data containing descriptors
# Dropping the solvent 0 and solvent 1 names columns since they've been hot encoded.
final_encoded = final_encoded.drop(columns=['Solvents_0', 'Solvents_1', 'Solvent Ratio_1', 'Solvent Ratio_2'])

final_encoded

# %%
#============================================================================
# One-hot encode other single-valued categorical columns
#============================================================================
...
other_categorical_cols = [c for c in other_categorical_cols if c in final_encoded.columns]

# Fill missing with "None" for these columns too
for c in other_categorical_cols:
    final_encoded[c] = final_encoded[c].fillna("None")

ohe_other = OneHotEncoder(sparse_output=False, drop='first')
cat_df = final_encoded[other_categorical_cols].astype(str)
...
final_encoded.drop(columns=other_categorical_cols, inplace=True)

# Combine categorical column encoding to the final_encoded containing solvent encoding by ratio and seed data preprocessing
final_encoded = pd.concat([final_encoded.reset_index(drop=True), encoded_other_df], axis=1)

# %%
# Ready for Sci kit learns iterative imputer to fill missing data so ML can use those columns!
final_encoded

# %%
# Define the XGBoost Regressor  -- XgBoost may be overfitting during imputation but I assume its same or better than mean filling imputation.
...
# You give iterative imputer all columns. Columns containing missing values are filled based on all columns that don't contain missing values in a round robin fashion.
impute_cols = [
#     # Already imputed
#     "Temperature (Celsius)",
...
#     # These are all the solvent ratios below in hot encoded fashion
#     'Hexane',
#     'Tetrahydrofuran',
#     'Dichloromethane',
#     'Acetonitrile',
...
iteratImputer_columns = final_encoded.drop(columns="Polymer")
# Adding all the columns to be used in iterative imputer here - I realized doing it manually I losted two columns somewhere.
impute_cols.extend(list(iteratImputer_columns))

# %%
# Filter out any that might not exist in 'data'
impute_cols = [c for c in impute_cols if c in final_encoded.columns]

# This prints all our columns - including ones that have no missing values used to fill missing values
print("Columns to be imputed with IterativeImputer:", impute_cols)
print(len(list(impute_cols)))

# %%
# Create a copy of your data to store the imputed results
data_imputed = final_encoded.copy()

# Subset the columns we want to impute
# This is here because at one point I was leaving the transient imputed columns out - pore size 2 / particle diameter 2
subset_df = final_encoded[impute_cols]
...
# I may consider raising the tolerance to 1e-1 which I believe would converge.
# Currently, does not converge in 100 iterations.
imputer = IterativeImputer(estimator=xgb_regressor, random_state=42, max_iter=150,tol=1e-2, verbose=2)
...
# %%
# Now data_imputated has no missing values and is ready for any machine learning model!
final_output_file = "polystyrene-imputated-solvents-hot-encoded-2-23-25.xlsx"
